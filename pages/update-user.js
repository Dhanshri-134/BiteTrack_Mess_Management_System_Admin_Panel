...
//   // Filter user by name, email, or mobile
//   const handleSearch = () => {
//     if (!searchTerm.trim()) return toast("Enter name, email, or mobile");
...
//       course: found.course || "",
//       parent_name: found.parents?.[0]?.name || "",
//       parent_contact: found.parents?.[0]?.contact || "",
//       parent_address: found.parents?.[0]?.address || "",
//     });
//   };
...
//         method: "PUT",
//         headers: { "Content-Type": "application/json" },
//         body: JSON.stringify({ id: filteredUser.id, ...form }),
//       });
//       const userData = await resUser.json();
//       if (!resUser.ok) return toast(userData.error || "Failed to update user");
...
//       });
//       const parentData = await resParent.json();
//       if (!resParent.ok) return toast(parentData.error || "Failed to update parent");
...
//                 {filteredUser.email || filteredUser.mobile})
//               </h2>
