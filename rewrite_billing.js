...
const code = `import { API_BASE } from "../../lib/api";
...
export default function BillsPage() {
  ...
  const openAdvanceModal = async (bill) => {
    try {
      const res = await fetch(
        \`\${API_BASE}/api/advance/fetch/?user_id=\${bill.user_id}&month=\${bill.month || month}&year=\${bill.year || year}\`,
        { headers: authHeaders() }
      );
  ...
  const openWhatsAppDrawer = (bill) => {
    ...
    const defaultMessage = \`Hello \${bill.name},\\n\\nYour mess bill status has been updated.\\n\\nThank you.\`;
  ...
  const groupedBills = (() => {
    ...
    bills.forEach((b) => {
      ...
      const isCurrent = month && year ? Number(b.month) === Number(month) && Number(b.year) === Number(year) : false;
  ...
  const downloadExcel = () => {
    ...
    const url = \`/api/bills/download/?month=\${month}&year=\${year}&token=\${token}\`;
                        ...
                        <tbody>
                          {filtered.map((b, idx) => (
                            <tr key={b.user_id}>
                              ...
                              <td title={b.note || ""}>{b.note ? b.note.slice(0, 25) + (b.note.length > 25 ? "…" : "") : "—"}</td>
                                ...
                                <div className={styles.actionMenuWrapper}>
                                  ...
                                  {openActions === b.user_id && (
                                    <div className={styles.actionDropdown} onClick={(e) => e.stopPropagation()}>
                                      ...
                                      <button onClick={() => { setExpandedCard(b.user_id); setOpenActions(null); }}>{t("paymentHistory")}</button>
                        ...
                        <div key={b.user_id || idx} className={styles.mobileCard}>
                          <div className={styles.cardHeader}>
                            ...
                            <button onClick={() => setExpandedCard(expandedCard === b.user_id ? null : b.user_id)}>
                              <ChevronDown size={24} style={{ transform: expandedCard === b.user_id ? "rotate(180deg)" : "rotate(0deg)", transition: "0.2s" }} />
                            </button>
                          </div>
                          ...
                          <div className={styles.cardBody}>
                            <div className={styles.cardRow}>
                                <span>{t("phone")}</span>
                                <strong><a href={\`tel:\${b.mobile}\`} style={{ color: "black", textDecoration: "none" }}>{b.mobile || "-"}</a></strong>
                            </div>
                          </div>
                          ...
                          {expandedCard === b.user_id && (
                            <div className={styles.cardBody} style={{ borderTop: "1px solid #e5e7eb", paddingTop: "12px", marginTop: "4px" }}>
                              ...
                              <div className={styles.cardRow}><span>{t("status")}</span><strong>{b.has_pending ? t("pending") : t("clear")}</strong></div>
                                ...
                                {b.history.sort((a, y) => { if(a.year === y.year) return y.month - a.month; return y.year - a.year }).map(h => (
                                  <div key={\`\${h.user_id}-\${h.year}-\${h.month}\`} style={{ background: "white", padding: "12px", marginBottom: "8px", borderRadius: "6px", border: "1px solid #e5e7eb" }}>
                                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                                      ...
                                      <span style={{ color: h.paid ? "#10b981" : "#ef4444", fontWeight: "600" }}>₹{getBillAmount(h).toFixed(2)}</span>
                                    </div>
                                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "#6b7280", marginBottom: "12px" }}>
                                      <span>{h.start_date || "-"} to {h.end_date || "-"}</span>
                                      <span>{h.paid ? t("paid") : t("pending")}</span>
                                    </div>
                                    <button 
                                      onClick={() => openPaymentModal(h, true)}
                                      style={{ width: "100%", padding: "8px", background: h.paid ? "#f3f4f6" : "#2563EB", color: h.paid ? "black" : "white", border: "none", borderRadius: "6px" }}>
                              ...
                              <div className={styles.cardActions} style={{ marginTop: "12px" }}>
                                <button className={\`\${styles.btnAction} \${styles.btnCalendar}\`} onClick={() => setSelectedAttendance({ year: b.year, month: b.month, attendanceMap: b.attendance_map || {}, ownerMarkedDates: b.owner_marked_dates || [], name: b.name })}>
                                  {t("viewCalendar")}
                                </button>
                              </div>
                            </div>
              ...
              {paymentModal && (
                <div className={styles.modalOverlay}>
                  <div className={styles.modalContent}>
                    <h3>{paymentModal.paid ? "Edit Payment" : t("markPayment")} — {paymentModal.name || paymentModal.user_name} ({paymentModal.month}/{paymentModal.year})</h3>
            ...
            <div className={styles.waOverlay} onClick={closeWhatsAppDrawer}>
              <div className={styles.waModal} onClick={(e) => e.stopPropagation()}>
                ...
                <div className={styles.waButtons}>
                  <button className={styles.waSend} onClick={() => sendWhatsApp(waDrawer.mobile)}>Student ({waDrawer.mobile})</button>
                  {waDrawer.parent_mobile && <button className={styles.waSend} onClick={() => sendWhatsApp(waDrawer.parent_mobile)}>Parent ({waDrawer.parent_mobile})</button>}
