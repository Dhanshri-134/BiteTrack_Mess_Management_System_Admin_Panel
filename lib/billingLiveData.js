import { normalizeAttendanceMap } from "./attendanceMap";
...
function isInactiveStatus(status) {
  ...
  return normalizedStatus === "inactive" || normalizedStatus === "frozen";
}

function getEffectiveMonthWindow(user, { monthStart, monthEnd, respectStatusTransitions = true }) {
  ...
  if (respectStatusTransitions) {
    const freezeDate = toDate(user.freeze_date);
    const unfreezeDate = toDate(user.unfreeze_date);

    if (isInactiveStatus(user.status) && freezeDate && freezeDate <= effectiveEnd) {
      const lastBillableDate = shiftUtcDate(freezeDate, -1);
      if (lastBillableDate && lastBillableDate < effectiveEnd) {
        effectiveEnd = lastBillableDate;
      }
    }

    if (!isInactiveStatus(user.status) && unfreezeDate && unfreezeDate > effectiveStart) {
      effectiveStart = unfreezeDate;
    }
  }
  ...
  return {
    effectiveStart,
    effectiveEnd,
    effectiveStartISO: isoDate(effectiveStart),
    effectiveEndISO: isoDate(effectiveEnd),
    hasOverlap: effectiveStart <= effectiveEnd,
  };
}
...
function buildMonthlyAttendanceMap({
  ...
  return buildDateRange(monthStartISO, monthEndISO, (dateKey) => {
    if (todayISO && dateKey > todayISO) return null;
...
export async function getLiveMonthlyBillingRows(db, { messId, month, year }) {
  ...
  const sourceUsersQuery = `
    WITH source_users AS (
      ...
      FROM attendance
      WHERE mess_id = $1
        AND EXTRACT(YEAR FROM att_date)::int = $2
        AND EXTRACT(MONTH FROM att_date)::int = $3
  ...
  const [
    ...
    db.query(attendanceQuery, [messId, monthStartISO, monthEndISO]),
    db.query(ownerMarkedQuery, [messId, monthStartISO, monthEndISO]),
    db.query(sourceUsersQuery, [messId, numericYear, numericMonth]),
  ]);

  const sourceUserIds = new Set(sourceUserRows.map((row) => String(row.user_id)));
  ...
  const rawPresentDatesByUser = new Map();
  const ownerMarkedByUser = new Map();

  for (const row of attendanceRows) {
    const userKey = String(row.user_id);
    if (!rawPresentDatesByUser.has(userKey)) {
      rawPresentDatesByUser.set(userKey, new Set());
    }

    rawPresentDatesByUser.get(userKey).add(row.att_date);
  }
  ...
  return users
    .filter((user) => {
      ...
      if (sourceUserIds.has(userKey)) return true;

      return getEffectiveMonthWindow(user, {
        monthStart,
        monthEnd,
        respectStatusTransitions: true,
      }).hasOverlap;
    })
    .map((user) => {
    const userKey = String(user.user_id);
    ...
    const hasSourceData = sourceUserIds.has(userKey);
    const { effectiveStartISO, effectiveEndISO } = getEffectiveMonthWindow(user, {
      monthStart,
      monthEnd,
      respectStatusTransitions: !hasSourceData,
    });
    const storedAttendanceMap = storedAttendance?.attendance_map
      ? normalizeAttendanceMap(storedAttendance.attendance_map)
      : null;
    const storedPresentDates = Object.keys(storedAttendanceMap || {})
      .filter((dateKey) => storedAttendanceMap[dateKey] === true)
      .sort();
    const rawPresentDates = Array.from(rawPresentDatesByUser.get(userKey) || []).sort();
    const presentDates = rawPresentDates.length > 0 ? rawPresentDates : storedPresentDates;
    const firstAttendanceISO =
      presentDates[0] || isoDate(storedAttendance?.first_attendance_date) || null;
    const billingStartISO = firstAttendanceISO || effectiveStartISO;
    const billingEndISO = effectiveEndISO;

    const derivedAttendanceMap = buildMonthlyAttendanceMap({
        monthStartISO,
        monthEndISO,
        firstAttendanceISO,
    ...
    const combinedAttendanceMap =
      rawPresentDates.length > 0 ? derivedAttendanceMap : storedAttendanceMap || derivedAttendanceMap;
    ...
    const totalAmount = Number(
      Number(savedBill?.total_amount ?? daysBilled * chosenPerDayRate).toFixed(2)
    );
    ...
    return {
      ...
      days_present: Number(
        rawPresentDates.length > 0
          ? rawPresentDates.length
          : storedAttendance?.days_present ?? presentDates.length
      ),
      ...
      pending_amount: Math.max(0, totalAmount - Number(payment?.amount || 0)),
...
export async function getUserMonthSourceKeys(db, messId) {
  const query = `
    ...
    WHERE sk.user_id IS NOT NULL
      ...
      AND (
        u.date_of_joining IS NULL
        OR make_date(CAST(sk.year AS int), CAST(sk.month AS int), 1) >= date_trunc('month', u.date_of_joining)::date
      )
    ORDER BY sk.year DESC, sk.month DESC, sk.user_id
  `;
