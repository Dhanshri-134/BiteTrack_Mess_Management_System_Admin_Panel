...
export async function getLiveMonthlyBillingRows(db, { messId, month, year }) {
  ...
  const attendanceQuery = `
    SELECT user_id, TO_CHAR(att_date, 'YYYY-MM-DD') AS att_date, source_type
    FROM (
      SELECT user_id, att_date, 'scan' AS source_type
      FROM attendance
      WHERE mess_id = $1
      ...
      SELECT user_id, att_date, 'owner' AS source_type
  ...
  for (const row of attendanceRows) {
    ...
    if (!attendanceByUser.has(userKey)) {
      attendanceByUser.set(userKey, {
        scanDates: new Set(),
        ownerDates: new Set(),
    ...
    const bucket = attendanceByUser.get(userKey);
    const dateKey = row.att_date;

    if (row.source_type === "owner") {
      bucket.ownerDates.add(dateKey);
    } else {
      bucket.scanDates.add(dateKey);
    }

    bucket.presentDates.add(dateKey);
  }
  ...
  return users.map((user) => {
    ...
    const attendance = attendanceByUser.get(userKey) || {
      scanDates: new Set(),
      ownerDates: new Set(),
      presentDates: new Set(),
    };
    ...
    const presentDates = Array.from(attendance.presentDates).sort();
    const firstAttendanceISO = presentDates[0] || null;
    const billingStartISO = firstAttendanceISO || effectiveStartISO;
    const billingEndISO = effectiveEndISO;

    const combinedAttendanceMap = buildMonthlyAttendanceMap({
      monthStartISO,
      monthEndISO,
      firstAttendanceISO,
    ...
    const daysBilled = calculateDaysBilled(
    ...
    const ownerMarkedDates = Array.from(attendance.ownerDates).sort();
    ...
    return {
      ...
      days_present: presentDates.length,
